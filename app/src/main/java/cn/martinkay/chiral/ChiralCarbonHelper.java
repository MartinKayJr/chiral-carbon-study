...
public class ChiralCarbonHelper {
    ...
    public static boolean isChiralCarbon(Molecule mol, @IndexFrom(1) int index) {
        ...
        for (int i = 0; i < bonds.length; i++) {
            ...
            if ("H".equals(mol.getAtom(another).element)
                && mol.getAtomDeclaredBonds(another).length == 1) {
                hcnt++;
                bonds[i] = null;
            } else {
                bondnh.add(b);
    ...
    public static boolean compareChain(Molecule mol, int center, int chain1, int chain2) {
        return compareChain(mol, center, center, chain1, chain2,
            (int) (3 + Math.sqrt(mol.atomCount())));
    }

    @IndexFrom(1)
    public static boolean compareChain(Molecule mol, int atom1, int atom2, int chain1, int chain2,
        int ttl) {
